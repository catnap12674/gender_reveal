(() => {
  ...
  async function loadGender() {
    ...
    if (gender === "boy" || gender === "girl") {
      ...
      // 注: サイズ指定(.baby-illustration の width:62%)は #babyIllustration 自身に
      // 直接あてる。入れ子のdivに付けると、親(#babyIllustration)の幅が
      // flexアイテムとして未確定(auto)のままパーセント指定を解決できず、
      // 0幅になってイラストが描画されない不具合があったため。
      babyIllustration.className = "baby-illustration";
      babyIllustration.innerHTML = window.babySvgMarkup(gender);
    } else {
      stageBg.className = "stage-bg pending";
      babyIllustration.className = "";
